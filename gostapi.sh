...
install_gost_and_setup() {
  ...
  # ---------- 2) 架构检测 ----------
  local UNAME_M ARCH_LABEL ARCH_FILE_SUFFIX
  UNAME_M=$(uname -m 2>/dev/null || echo "x86_64")
  case "$UNAME_M" in
    x86_64|amd64) ARCH_LABEL="linux_amd64"; ARCH_FILE_SUFFIX="linux_amd64" ;;
    aarch64|arm64) ARCH_LABEL="linux_arm64"; ARCH_FILE_SUFFIX="linux_arm64" ;;
...
# ---------- 3) 智能镜像选择 ----------
  ...
  echo "正在获取最新版本信息..."
  local latest_json=""
  latest_json=$(curl -fsSL --max-time 5 "${api_url}" 2>/dev/null || echo "")
  ...
  # 回退机制
  if [ -z "$asset_url" ]; then
      echo "⚠️  无法通过 API 获取最新版本 (GitHub API 受限)。"
      local FALLBACK_VER="v3.2.6"
      tag_name="${FALLBACK_VER} (Fallback)"
      asset_url="https://github.com/go-gost/gost/releases/download/${FALLBACK_VER}/gost_${FALLBACK_VER#v}_${ARCH_FILE_SUFFIX}.tar.gz"
      echo "🔄 切换到回退版本: ${FALLBACK_VER}"
  fi

  echo "准备下载版本: ${tag_name}"
  ...
  echo "安装到 ${dest} ..."
  $SUDO install -m 0755 "$gost_candidate" "$dest" || { echo "install 失败"; rm -rf "$tmpdir" || true; return 7; }
...
EOF
    ...
    $SUDO systemctl restart gost.service >/dev/null 2>&1 || true
    ...
    if [ "$api_code" = "200" ]; then
        ...
        echo "⚠️  GOST 已安装，但 API 无法访问 (HTTP ${api_code})。请检查 logs。"
    fi
  else
    echo "⚠️  未检测到 systemd，已仅安装二进制文件。请手动启动。"
  fi
...
least_ping_auto() {
  ...
  if [ -n "$arg_port" ]; then
      ...
      read -e -rp "请输入 GOST 监听端口 (例如 44411): " LISTEN_PORT
  ...
  local TARGET_2
  while true; do
      read -e -rp "请输入备选落地 B (格式 IP:PORT): " TARGET_2
...
# 依赖检查
if ! command -v python3 >/dev/null 2>&1; then log "Error: python3 missing"; exit 1; fi
if ! command -v jq >/dev/null 2>&1; then log "Error: jq missing"; exit 1; fi
...
if [ "\$p1" == "99999" ] && [ "\$p2" == "99999" ]; then log "[ERROR] All targets unreachable. Skip."; exit 1; fi
...
if [ "\$FORCE_MODE" == "false" ]; then
    if [ "\$ACT" -gt 0 ]; then
        log "[SKIP] 发现更优节点 \$WINNER (A:\${p1}ms/B:\${p2}ms)，但端口繁忙 (活跃连接: \$ACT)。保持当前节点 \$CUR 不变。"
        exit 0
    fi
fi

# 执行切换
log "[SWITCH] 触发切换: \$CUR -> \$WINNER (延迟优势 A:\${p1}ms/B:\${p2}ms | 当前连接: \$ACT)"
...
# 自动保存
CONFIG_DATA=\$(curl -s \$H "\${API_URL}/config")
if echo "\$CONFIG_DATA" | jq empty >/dev/null 2>&1; then
    echo "\$CONFIG_DATA" | jq '.' > /etc/gost/config.json 2>/dev/null
    log "[SUCCESS] 配置更新完成。"
fi
EOF
  chmod +x "$task_script"
  ...
  echo "✅ Crontab 任务已添加！"; echo "   日志文件: ${log_file}"; pause
}
...
delete_forward() {
  ...
  local -a ports_to_clean=()

  # [关键修复] 使用 | 作为分隔符，防止 Tab/空格 混淆导致列错位
  while IFS='|' read -r s_name s_chain s_addr; do
    ...
    if [ "$s_name" = "${svc_base}-tcp" ] || [ "$s_name" = "${svc_base}-udp" ] || echo "$s_name" | grep -Fq "$svc_base"; then
        ...
        # 记录 Chain (增加过滤：不为空，且看起来不像 IP 地址)
        if [ -n "$s_chain" ] && [ "$s_chain" != "null" ]; then
            # 排除包含冒号的字符串（通常是IPv6或IP:Port），因为Chain名通常不含冒号
            if [[ "$s_chain" != *":"* ]]; then
        ...
        # 提取端口
        local p
        p=$(echo "$s_addr" | awk -F: '{print $NF}')
        if [[ "$p" =~ ^[0-9]+$ ]]; then
            if [[ ! " ${ports_to_clean[*]} " =~ " ${p} " ]]; then
                ports_to_clean+=("$p")
            fi
        fi
    fi
  done < <(echo "$all_services_json" | jq -r '.[] | "\(.name)|\(.handler.chain // "")|\(.addr // "")"')
  ...
  if [ "${#ports_to_clean[@]}" -gt 0 ]; then
      for p in "${ports_to_clean[@]}"; do
          local task_script="/etc/gost/tasks/monitor_${p}.sh"
          local task_log="/var/log/gost_monitor_${p}.log"
          
          if crontab -l 2>/dev/null | grep -q "monitor_${p}.sh"; then
              echo "  🧹 清理端口 $p 的监控任务..."
              (crontab -l 2>/dev/null | grep -v "monitor_${p}.sh") | crontab -
          fi
          
          if [ -f "$task_script" ]; then rm -f "$task_script"; fi
          if [ -f "$task_log" ]; then rm -f "$task_log"; fi
      done
  fi
...
══════════════════════════════════════════════════════════
           GOST API 管理工具 V1.3.4 2025/11/24
仓库地址：https://github.com/lengmo23/Gostapi_forward
V1.3.4 Leastping均衡,转发Relay链复用,中转机Github连接镜像
══════════════════════════════════════════════════════════
$(get_gost_status)
